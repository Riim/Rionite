...
[jsfiddle](http://jsfiddle.net/a2u9pfsa/)

```html
<div rt-view="eventExample">Title</div>
...
Возвращает объект с методом `stop`, при вызове которого обработчик будет снят (если один `listenTo` добавляет сразу несколько обработчиков, то снимуться сразу все).
...
Возвращает объект с методом `clear`, при вызове которого таймер будет остановлен.
...
Сложное взаимодействие между несколькими въюшками обычно делается через модель приложения, но простейшее взаимодействие, видящих напрямую друг-друга въюшек, обычно допустимо и удобнее делать без использования модели.
...
Позволяет отправить сигнал от въюшки-предка к въюшке-потомку. Первый аргумент - имя метода, остальные - передаваемые в метод аргументы. Если въюшка-потомок не содержит нужного метода, она пропускается.  
